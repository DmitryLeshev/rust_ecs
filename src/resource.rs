...
mod test {
    ...
    fn add_resource() {
        ...
        let extracted_wordl_width = stored_resource.downcast_ref::<WorldWidth>().unwrap();

        assert_eq!(extracted_wordl_width.0, 100.0);
    }

    #[test]
    fn get_resource() {
        let resources = initialize_resource();
        let extracted_wordl_width = resources.get_ref::<WorldWidth>().unwrap();
        assert_eq!(extracted_wordl_width.0, 100.0);
    }
